...
const FIELD_HELP: Record<string, string> = {
  ...
  'api:service.AlertsEnabled': 'Enable the alerting subsystem. When false (default), the alert API routes are not registered and the alert tables are not created. When true, the API serves alert rule/channel management and history from the shared database (evaluated by osctrl-tls). Requires a service restart to change.',
  'tls:service.AlertsEnabled': 'Enable the alerting subsystem. When false (default), alert rules are not evaluated on the log ingest path and the alert tables are not created. When true, osctrl-tls loads alert rules at boot and matches ingested logs against them. Requires a service restart to change.',
  ...
  'saml.LinkLocalAccounts': 'Let a SAML login claim an existing LOCAL password account with the same username, instead of refusing it. Off by default, because with it on whoever controls the IdP’s username namespace can take over any same-named local account, admins included. Turn it on when you pre-create accounts locally and want the IdP to adopt them — the first such login stamps the row as federated and is written to the audit log, and the flag is no longer needed for that account afterwards.',
  'saml.UsernameAttribute': 'Attribute (Name or FriendlyName) whose value becomes the osctrl username. Empty = use the NameID verbatim, which is usually a mailbox and is fine — usernames may be either a plain handle (^[a-zA-Z0-9_-]{1,64}$) or an email address. Email usernames are lowercased so identity does not depend on the casing the IdP emits.',
  ...
  'oidc.UsernameClaim': "Empty defaults to preferred_username. `email` is supported and is lowercased when stored; it is only accepted when the IdP also sets email_verified, so an unverified address cannot claim an account. Auth0's `sub` (e.g. `auth0|abc123`) is still rejected — use `nickname` or `email` there.",
  ...
  'oidc.LinkLocalAccounts': 'Let an OIDC login claim an existing LOCAL password account with the same username, instead of refusing it. Off by default, because with it on whoever controls the IdP’s username namespace can take over any same-named local account, admins included. Turn it on when you pre-create accounts locally and want the IdP to adopt them — the first such login stamps the row as federated and is written to the audit log, and the flag is no longer needed for that account afterwards.',
