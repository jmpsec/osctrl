# Using Docker to develop Osctrl
The following Docker compose `docker-compose-dev.yml` will build an environment for development. Each Osctrl component is a [golang](https://hub.docker.com/_/golang) container using [air](https://github.com/cosmtrek/air) to autoload changes made locally and compile them. In addition, this docker-compose exposes each container of the Docker stack on loopback interface of the development machine. There a couple of manual steps that are required before having a fully functional deployment:
...
1. `vim .env` and set:
    ...
    1. Development
        1. `GOLANG_VERSION` - Define the version of golang to use
        1. `AIR_VERSION` - Define the version of AIR to auto load code changes and compile them
    1. Save and exit    
